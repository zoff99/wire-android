...
public class AudioMessageViewControllerTest extends ViewTest<MainTestActivity> {


    public AudioMessageViewControllerTest() {
    ...
    public void verifyProgressIndicatorVisibleForSenderWhileUploading() {
        ...
        assertTrue(viewController instanceof AudioMessageViewController);
    ...
    @Test
    public void verifyCancelAudioUpload() throws InterruptedException {
        Message message = createMockMessage(Message.Status.PENDING);
        User user = createMockUser();
        Asset asset = createMockAsset(AssetStatus.UPLOAD_IN_PROGRESS);
        ProgressIndicator progressIndicator = mock(ProgressIndicator.class);
    ...
    public void verifyProgressIndicatorVisibleWhileDownloading() throws InterruptedException {
        ...
        ProgressIndicator progressIndicator = mock(ProgressIndicator.class);
        when(progressIndicator.getProgress()).thenReturn(50L);
        when(progressIndicator.getTotalSize()).thenReturn(100L);
        when(progressIndicator.getState()).thenReturn(ProgressIndicator.State.RUNNING);
        ViewControllerMockHelper.setupMessageMock(message, user, asset, progressIndicator, activity);
        ...
        onView(withId(R.id.aab__row_conversation__audio_button)).check(isVisible());
        GlyphProgressView glyphProgressView = ViewUtils.getView(activity, R.id.aab__row_conversation__audio_button);
        assertTrue("Progress view is not animating", Float.compare(glyphProgressView.getProgress(), 0.5F) == 0);
    }


    @Test
    public void verifyCancelAudioDownload() throws InterruptedException {
        Message message = createMockMessage(Message.Status.SENT);
        User user = createMockUser();
        Asset asset = createMockAsset(AssetStatus.DOWNLOAD_IN_PROGRESS);
        ProgressIndicator progressIndicator = mock(ProgressIndicator.class);
    ...
    @Test
    public void verifyDownloadFail() {
        Message message = createMockMessage(Message.Status.SENT);
        User user = createMockUser();
        Asset asset = createMockAsset(AssetStatus.DOWNLOAD_FAILED);
        ViewControllerMockHelper.setupMessageMock(message, user, asset, activity);
    ...
    @Test
    public void verifyRetryAudioDownload() throws InterruptedException {
        Message message = createMockMessage(Message.Status.SENT);
        User user = createMockUser();
        Asset asset = createMockAsset(AssetStatus.DOWNLOAD_FAILED);
        ProgressIndicator progressIndicator = mock(ProgressIndicator.class);
        when(progressIndicator.getState()).thenReturn(ProgressIndicator.State.FAILED);
        ViewControllerMockHelper.setupMessageMock(message, user, asset, progressIndicator, activity);
    ...
    private Asset createMockAsset(AssetStatus status) {
        ...
        when(asset.getName()).thenReturn("audio_recording.mp3");
    ...
    private Message createMockMessage(Message.Status status) {
        ...
        when(message.getMessageType()).thenReturn(Message.Type.AUDIO_ASSET);
