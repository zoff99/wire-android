...
/**
 * Changes compared to ViewDragHelper from the Support lib:
 * - replace {@link sInterpolator} with an {@link com.waz.zclient.utils.animation.interpolators.penner.expo.Expo.EaseOut}
 *   interpolator
 * - change duration computation in {@link this#computeSettleDuration(android.view.View, int, int, int, int)}
 * - change values of {@link this#BASE_SETTLE_DURATION} and {@link this#MAX_SETTLE_DURATION}
 *
 *
 *
...
public class ImageDragViewHelper {
    private static final String TAG = "ImageDragViewHelper";
    ...
    private static final int BASE_SETTLE_DURATION = 50; // ms
    private static final int MAX_SETTLE_DURATION = 150; // ms
     ...
     */
    private static final Interpolator sInterpolator = new Expo.EaseOut();
     ...
     */
    public static ImageDragViewHelper create(ViewGroup forParent, Callback cb) {
        return new ImageDragViewHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static ImageDragViewHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final ImageDragViewHelper helper = create(forParent, cb);
     ...
     * @param forParent Parent view to monitor
     */
    private ImageDragViewHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        if (velocity > 0) {
            duration = 2 * Math.round(1000 * Math.abs(distance / velocity));
        } else {
